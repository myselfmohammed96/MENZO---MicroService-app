...
const loadSelectFields = async () => {
    try {
        ...
        if (!Array.isArray(discountStatus)) {
            throw new Error("Invalid data format: discount status should be array");
        }

        //  populating data
        populateDiscountLevels(discountLevels);
        populateDiscountTypes(discountTypes);
        populateCapTypes(capTypes);
        populateDiscountStatus(discountStatus);
