...
const urls = {
    getCategories: `${apiBase}/get-all-parents`,
    getSub: `${apiBase}/get-all-sub`,
...
//  ******* Save new sub-option *******
    ...
    async function loadCategories() {
        try{
            const response = await fetch(urls.getCategories, {
                method: "GET",
                credentials: "include"
            });
            if (!response.ok) throw new Error("Failed to load categories");
            const categories = await response.json();
            categories.forEach(c => addCategory(c.id, c.categoryName));
        } catch (error) {
    ...
    async function addCategory(categoryId = null, categoryName = "") {
...
//                    });
        ...
        header.onclick = async () => {
            ...
            if(accordion.dataset.loaded === "false") {
                let added = addSubCategories(categoryId, content);
                console.log(added);
                if(added) {
                    accordion.dataset.loaded = "true";
                    content.style.display = "block";
                } else {
                    alert("Unable to load sub-categories. Try again later.");
                }
            } else {
    ...
    async function addSubCategories(categoryId, content) {
        console.log("sub-executing for ID: ", categoryId);
        try {
            const response = await fetch(`${urls.getSub}?id=${categoryId}`, {
                method: "GET",
                credentials: "include"
            });
            if(!response.ok) throw new Error("Failed to load sub-categories for category ID: ", categoryId);
    ...
    function addSubCategory(subCategoryId, subCategoryName) {
