...
// Save sub-option
async function saveSubCategory(name, categoryId) {
    try {
        const response = await fetch(urls.addSub, {
            method: "POST",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ parentCategoryId: categoryId, categoryName: name })
        });
        if (!response.ok) {
            const err = await response.json().catch(() => ({}));
            throw new Error(err.message || "Failed to save sub-option");
        }
        return await response;
    } catch (error) {
        console.error("Save Sub Option Error: ", error);
        alert("Failed to save sub-option.");
        throw error;
    }
}
...
function openAddSubCategoryModal(categoryId = null) {
    ...
    saveBtn.onclick = async function (event) {
        event.stopPropagation();
        if (categoryId) await saveAddSubCategoryModal(categoryId);
    }
}
...
async function saveAddSubCategoryModal(categoryId = null) {
    const name = document.getElementById("sub-modal-category-input").value.trim();
    if (!name) return;
    try {
        if (categoryId) {
            const response = await saveSubCategory(name, categoryId);
